...
class SpecDay extends GetxController {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        for (int i = 0; i < helper.length; i++) {
          ...
          inte = [];
          for (int j = 0; j < helperint.length; j++) {
            Ingredient test = Ingredient(
              name: helperint[j]["name"],
              image: helperint[j]["image"],
            );
            inte.add(test);
          }
  ...
  Future getexer() async {
    ...
    exer = [];
    String url =
        '${Constans.baseUrl}exercise/exercisePlan/${coachid.toString()}';
    try {
      final res = await http.post(Uri.parse(url),
          headers: {
            'Accept': 'application/json',
...
// ignore: non_constant_identifier_names
  ...
  Future unlockedday() async {
    ...
    try {
      ...
      print({"choose": equ, "coach_id": coachid.toString()});
      final resdata = json.decode(res.body);
      if (res.statusCode == 200) {
      } else {
        throw "Something wrong , please try again";
